...
void initParticles(std::vector<Particle>& particles, uint N_sph, double alpha, double beta);
...
// void getAcceleration_old(std::vector<Particle>& particles, double h, uint N_sph);
double MomentumViscosity(std::vector<Particle>& particles,uint i,uint index);
void getChangeInInternalEnergy(std::vector<Particle>& particles, double h, uint N_sph);
double InternalEnergyViscosity(std::vector<Particle>& particles,uint i,uint index);
...
int main(){
    ...
    //alpha for viscosity term of momentum equation
    double alpha_global=1.0;
    //beta for signal velocity
    double beta_global=2.0*alpha_global;
    // Stopping time
    double stop_t=0.05;
    // set initial time
    double init_t=0.0;
    ...
    initParticles(particles, N_sph, alpha_global, beta_global);
    ...
    while (t<=stop_t)   
    {
        ...
        std::cout << "particles[0].internalEnergy = " << particles[0].internalEnergy << std::endl;
         std::cout << "particles[0].acceleration = " << particles[0].acceleration << std::endl;
...
// Old function to calculate the acceleration for each particle
// void getAcceleration_old(std::vector<Particle>& particles, double h, uint N_sph)
// {
//     // loop over all particles
//     for(uint i=0; i<N_sph; i++)
...
void getAcceleration(std::vector<Particle>& particles, double h, uint N_sph)
{
    ...
    for(uint i=0; i<N_sph; i++)
    {
        ...
        for(uint j=0; j<N_neighbours; j++)
        {
            ...
            // add the contribution of neighbours j to the acceleraton. In the beginning the acceleration is set to zero.
            // Calculate the acceleration with equation (1)
            double v_ij_test=(particles[i].velocity - particles[index].velocity);
            double temp1=(particles[i].pressure / particles[i].density / particles[i].density);
            double temp2=(particles[index].pressure / particles[index].density / particles[index].density);
            double temp3=MomentumViscosity(particles,i,index);
...
// Function to calculate the viscous term of the momentum equation, PI_ij
double MomentumViscosity(std::vector<Particle>& particles,uint i,uint index){
    // set the viscous term to 0.
    double visc_term=0.0;
    if (i!=index){
        //alpha
        double alpha = particles[i].alpha;
//         std::cout << "i,j= " << i << " " << index << "\n";
        //beta
        double beta = particles[i].beta;
        //std::cout << "beta= " << beta << "\n";
        //isothermal sound speed at i
        double c_i = sqrt(particles[i].internalEnergy);
    //     std::cout << "c_i= " << c_i << "\n";
        //isothermal sound speed at j
        double c_j = sqrt(particles[index].internalEnergy);
    //     std::cout << "c_j= " << c_j << "\n";
        //r_ij
        double r_ij = particles[i].position - particles[index].position;
    //     std::cout << "r_ij= " << r_ij << "\n";
        // r_ij_abs is the absolute value of r_ij
        double r_ij_abs = fabs(r_ij);
    //     std::cout << "r_ij_abs= " << r_ij_abs << "\n";
        // v_ij velocity difference
        double v_ij = particles[i].velocity - particles[index].velocity;
    //     std::cout << "v_ij= " << v_ij << "\n";
        // mean density
        double rho_ij = (particles[i].density + particles[index].density)/2.0;
    //     std::cout << "rho_ij= " << rho_ij << "\n";
        // signal velocity
        double v_sig = c_i + c_j - beta*v_ij*r_ij/r_ij_abs;
    //    std::cout << "v_sig= " << v_sig << "\n";
        // product of velocity difference and position difference
        double product = r_ij*v_ij;
        if (product<=0.0){
            visc_term+=alpha*v_sig*v_ij*r_ij / rho_ij / r_ij_abs;
        }
        else{
            visc_term=0.0;
        }
...
void getChangeInInternalEnergy(std::vector<Particle>& particles, double h, uint N_sph)
{
    ...
    for(uint i=0; i<N_sph; i++)
    {
        ...
        for(uint j=0; j<N_neighbours; j++)
        {
            ...
            if (index!=i){
                particles[i].changeInInternalEnergy += particles[index].mass * InternalEnergyViscosity(particles,i,index) * deriv_kernel_function(r_ij_abs, h) * sign_r_ij;}
            else{
                particles[i].changeInInternalEnergy +=0.0;
            }
...
//Function to calculate the viscosity term in the energy equation
double InternalEnergyViscosity(std::vector<Particle>& particles,uint i,uint index){
    // set the viscous term to 0.
    double visc_term=0.0;
    if (i!=index){
...
//         std::cout << "r_ij= " << r_ij << "\n";
    ...
    //     std::cout << "r_ij_abs= " << r_ij_abs << "\n";
        // v_ij velocity difference
        double v_ij = particles[i].velocity - particles[index].velocity;
//         std::cout << "v_ij= " << v_ij << "," << particles[i].velocity << "," << particles[index].velocity << "\n";
        // mean density
        double rho_ij = (particles[i].density + particles[index].density)/2.0;
    //     std::cout << "rho_ij= " << rho_ij << "\n";
        // signal velocity
    ...
    //    std::cout << "v_sig= " << v_sig << "\n";
        // product of velocity difference and position difference
        double product = r_ij*v_ij;
//         std::cout << "product1-->" << product << "\n";
        if(product <=0.0){
            visc_term=alpha*v_sig*v_ij*r_ij*v_ij*r_ij / 2.0 / rho_ij / r_ij_abs/ r_ij_abs;
        }
        else{
        visc_term=0.0;
        }
    }
    if (std::isnan(visc_term)){
        visc_term=0.0;
    }
//     std::cout << "visc_term1-->" << visc_term << "\n";
    return visc_term;
}
...
double getTimestep(std::vector<Particle>& particles, double h, double gamma, double gamma_CFL, uint N_sph)
{
    ...
    for(uint i=0; i<N_sph; i++)
    {
        ...
        double temp3 = h / ((1+1.2*particles[i].alpha)*c_i+(1+1.2*particles[i].beta)*h*grad_v);

        double temp4 = fmin(temp2,temp3);

        particles[i].timestep = gamma_CFL * fmin(temp1, temp4);
    }
